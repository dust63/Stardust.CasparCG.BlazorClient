...
namespace Stardust.Flux.Server.Controllers
{
    [ApiController]
    [Route("[controller]")]
    public class YoutubeController : ControllerBase
    {
        ...
        {
            var url = _authService.GetAuthorizationUrl(accountId ?? Guid.NewGuid().ToString(), name, YouTubeService.Scope.Youtube);
            return Redirect(url);
        }
        ...
        {
            return _authService.GetYoutubeAuthenticationToken(code);
        }
        ...
        [HttpGet]
        [Route("ChannelInfo")]
        public async Task<ActionResult<YoutubeChannelDto[]>> GetChannelInfo(CancellationToken cancellationToken,
         [FromQuery] string accountId)
        {
            if (accountId is null)
            {
            ...
            try
            {
                var channelInfo = await _youtubeService.GetChannelInfo( accountId, cancellationToken);
